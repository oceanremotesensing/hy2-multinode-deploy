...
# --- 用户配置 ---
# Cloudflare 中转IP (优选IP), 您可以随时修改这个IP并重新运行脚本来更新配置
...
echo "🔧 [2/8] 安装或更新 Hysteria..."
echo "  > 正在停止任何可能在运行的旧 Hysteria 服务..."
# 优雅地停止任何已存在的 systemd 服务，以释放文件锁。
# || true 确保在首次安装（服务不存在）时脚本不会因错误而退出。
systemctl stop 'hy2@*.service' || true
# 再次使用 pkill 作为双重保险，清理任何可能残留的进程。
pkill -f hysteria || true
# 等待1秒确保进程已完全退出
sleep 1

echo "  > 正在下载最新的 Hysteria 程序..."
